...
from backend_api.http.services import auth_service, chat_service

router = APIRouter(tags=["chats"])
ws_router = APIRouter(tags=["chats"])


def _require_session_access(user: User, session) -> None:
    if session is None:
...
def create_chat(
    ...
    session = chat_service.create_session(db, user, message=message)
...
def list_chats(
    ...
    return [
        ChatSessionListItem.model_validate(chat_service.session_to_list_item(row))
        for row in rows
    ]
...
def get_chat_messages(
    ...
    return [
        ChatMessageOut.model_validate(chat_service.message_to_out(m))
        for m in messages
    ]
...
async def chat_websocket(websocket: WebSocket, chat_id: int) -> None:
    ...
    try:
        ...
        await chat_service.manager.connect(chat_id, websocket)
        ...
        try:
            while True:
                ...
                if session is None:
                    await websocket.send_text(
                        json.dumps({"type": "error", "detail": "Chat not found"})
                    )
                    break
                if session.status == chat_service.CHAT_STATUS_CLOSED:
                    await websocket.send_text(
            ...
            await chat_service.manager.broadcast(
                chat_id,
                chat_service.presence_event_payload(chat_id, user_id=user.id, online=False),
            )
    finally:
        db.close()
